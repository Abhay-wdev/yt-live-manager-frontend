...
const StreamsManager = () => {
  ...
  const handleBulkAction = async (action: 'start' | 'stop' | 'delete') => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/stream/bulk/${action}`, { ids: selectedIds }, {
  ...
  const handleDuplicate = async (id: string) => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/stream/instances/${id}/duplicate`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (err) {
